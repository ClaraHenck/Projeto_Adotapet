...
$sql = "SELECT * FROM cachorros WHERE id_cachorro = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE cachorros SET id_ong=?, nome=?, raca=?, genero=?, porte=?, temperamento=?,
            idade=?, cidade=?, estado=?, rua=?, numero=?, descricao=? WHERE id_cachorro=?";
    $stmt = $pdo->prepare($sql);
...
<header>
    ...
    <h1>✏️ Editar Cachorro #<?php echo $id; ?></h1>
</header>
        ...
        <form method="post">
            ...
            <select name="id_ong" required>
                ...
                <option value="<?php echo $o["id_ong"]; ?>"
                    <?php echo $o["id_ong"] == $c["id_ong"] ? "selected" : ""; ?>>
            ...
            <div class="row">
                ...
                <div>
                    <label>Raça:</label>
                    <input type="text" name="raca" value="<?php echo htmlspecialchars($c["raca"]); ?>" required>
                </div>
            </div>
            ...
            <div class="row">
                <div>
                    ...
                    <select name="genero" required>
                        <option value="Macho"  <?php echo $c["genero"] == "Macho"  ? "selected" : ""; ?>>♂ Macho</option>
                        <option value="Femea"  <?php echo $c["genero"] == "Femea"  ? "selected" : ""; ?>>♀ Fêmea</option>
                    </select>
                </div>
                <div>
                    <label>Porte:</label>
                    <select name="porte" required>
                        <option value="Pequeno" <?php echo $c["porte"] == "Pequeno" ? "selected" : ""; ?>>Pequeno</option>
                        <option value="Medio"   <?php echo $c["porte"] == "Medio"   ? "selected" : ""; ?>>Médio</option>
                        <option value="Grande"  <?php echo $c["porte"] == "Grande"  ? "selected" : ""; ?>>Grande</option>
                    </select>
                </div>
                <div>
                    <label>Idade (meses):</label>
                    <input type="number" name="idade" min="0" value="<?php echo $c["idade"]; ?>" required>
                </div>
            </div>

            <label>Temperamento:</label>
            <input type="text" name="temperamento" value="<?php echo htmlspecialchars($c["temperamento"]); ?>" required>

            <label>Descrição:</label>
            <textarea name="descricao"><?php echo htmlspecialchars($c["descricao"]); ?></textarea>
            ...
            <div class="row">
                <div>
                    <label>Cidade:</label>
                    <input type="text" name="cidade" value="<?php echo htmlspecialchars($c["cidade"]); ?>" required>
                </div>
                <div>
                    <label>Estado:</label>
                    <input type="text" name="estado" value="<?php echo htmlspecialchars($c["estado"]); ?>" maxlength="2" required>
                </div>
            </div>
            ...
            <div class="row">
                <div>
                    <label>Rua:</label>
                    <input type="text" name="rua" value="<?php echo htmlspecialchars($c["rua"]); ?>" required>
                </div>
                <div>
                    <label>Número:</label>
                    <input type="text" name="numero" value="<?php echo htmlspecialchars($c["numero"]); ?>" required>
                </div>
            </div>
